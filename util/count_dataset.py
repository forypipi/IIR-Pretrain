...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
        ...
        for line in f_str:
            img, mask = line.split(' ')
            img = os.path.join(self.data_root, 'JPEGImages', os.path.basename(img))
            mask = os.path.join(self.data_root, 'SegmentationClassAug', os.path.basename(mask.strip()))
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.shot):
            ...
            while((support_image_path == image_path and support_label_path == label_path) or support_idx in support_idx_list):
                ...
                support_image_path = os.path.join(self.data_root, 'JPEGImages', os.path.basename(support_image_path))
                support_label_path = os.path.join(self.data_root, 'SegmentationClassAug', os.path.basename(support_label_path.strip()))
...
class BaseData(Dataset):
    def __init__(self, data_root=None, data_list=None, data_set=None, use_split_coco=False, transform=None):
    ...
    def __getitem__(self, index):
        ...
        image = np.float32(image)
        label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
        label_tmp = np.zeros_like(label, dtype=np.float32)

        cls_label_list = np.unique(label)
        ...
        cls = np.random.choice(cls_label_list)
        select_pix = np.where(label == cls)
        label_tmp[select_pix[0],select_pix[1]] = 1.0
        # image = image * np.repeat(np.expand_dims(label_tmp, 2), 3, 2)

        if self.transform is not None:
            image, label_tmp = self.transform(image, label_tmp)

        return image, label_tmp, cls