...
def cosine_warmup_lr(optimizer, base_lr, curr_iter, max_iter, warmup_iter=0, 
                     min_lr=0, index_split=-1, scale_lr=10.):
    """Cosine learning rate with warmup"""
    # Warmup phase
    if warmup_iter > 0 and curr_iter < warmup_iter:
        lr = base_lr * (curr_iter / warmup_iter)
    
    # Cosine annealing phase
    else:
        # Adjust iterations if warmup was used
        if warmup_iter > 0:
            iter_after_warmup = curr_iter - warmup_iter
            max_iter_after_warmup = max_iter - warmup_iter
        else:
            iter_after_warmup = curr_iter
            max_iter_after_warmup = max_iter
        
        # Calculate cosine annealing LR
        progress = min(iter_after_warmup / max_iter_after_warmup, 1.0)
        lr = min_lr + 0.5 * (base_lr - min_lr) * (1 + math.cos(math.pi * progress))

    # Apply different LRs to different parameter groups
    for index, param_group in enumerate(optimizer.param_groups):
        if index <= index_split:
            param_group['lr'] = lr
        else:
            param_group['lr'] = lr * scale_lr   # 10x LR for later layers
...
def get_save_path(args):
    args.snapshot_path = f'exp/{args.data_set}/{args.arch}/split{args.split}/{args.snapshot_path}'
    args.result_path = f'exp/{args.data_set}/{args.arch}/split{args.split}/{args.result_path}'
