...
class OneModel(nn.Module):
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        '''        
        cat_idx: the index of select class in sub_list (e.g. 0~15 in fold3)
        '''        
        x_size = x.size()
        bs = x_size[0]
        ...
        # nn.Conv2d(2, 1, k=1) initialize as [1,0]
        merge_map = torch.cat([meta_map_bg, base_map], 1)           # [bs, 2, 60, 60]
        merge_bg = self.cls_merge(merge_map)                        # [bs, 2, 60, 60] to [bs, 1, 60, 60]
